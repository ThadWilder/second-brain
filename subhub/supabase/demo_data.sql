...
--   • 1 project owned by subhub_qa@gmail.com (contractor)
--   • 4 jobs (3 open/posted, 1 claimed + in progress by Stephen Dianosaur)
--   • a natural message thread between them on the claimed job
--
-- Safe to run in the Supabase SQL editor (runs as service role → bypasses RLS).
-- Re-running creates a fresh copy; to wipe a prior run first see the cleanup
-- block at the very bottom (commented out).
...
begin
  ...
  if not exists (select 1 from contractor_profiles where user_id = v_contractor) then
    raise exception 'subhub_qa@gmail.com has no contractor_profiles row — finish contractor onboarding for that account first.';
  end if;
  ...
  if v_sub is null then
    raise exception 'Sub "Stephen Dianosaur" not found in sub_profiles.name. Check the spelling.';
  end if;
  ...
  -- Claimed + in-progress job (claimed by Stephen Dianosaur)
  insert into jobs (
    contractor_id, project_id, title, industry, scope_of_work,
    material_supplier, material_supplier_address, material_status,
    address, city, state, zip,
  ...
  -- Older messages marked read; last contractor message is unread so Stephen
  -- sees an unread badge when he logs in as the sub.
  insert into messages (job_id, sender_id, sender_role, body, created_at, read_at) values
    ...
    -- Latest message left unread → Stephen sees an unread badge on login
    (v_job_claimed, v_contractor, 'contractor',
     'Appreciate it. Inspection is booked for Thursday AM, so anytime Wednesday to wrap up works great.',
     now() - interval '20 minutes',
     null);

  raise notice 'Demo data created ✓ — project %, jobs (posted: % % %, in_progress: %)',
    v_project, v_job_posted1, v_job_posted2, v_job_posted3, v_job_claimed;
end $$;


-- ─────────────────────────────────────────────────────────────────────────────
-- CLEANUP: uncomment both lines and run to wipe a prior seed before re-running.
-- ─────────────────────────────────────────────────────────────────────────────
