...
- **Edge-swipe nav drawer & swipe-between-subtabs** (`components/GestureNav.tsx`) — the final two blueprint gestures, wrapping the tab content in both layouts. (1) **Edge-swipe drawer**: drag in from the left edge (or tap the thin accent grip) to slide out the full navigation list; dismiss by swiping it left, tapping the backdrop, or picking an item. (2) **Swipe-between-subtabs**: a horizontal fling across screen content moves to the adjacent PRIMARY tab (the four-item bottom-bar order). Native-only (`enabled={!isWeb}` — web uses the sidebar); renders children straight through when disabled. Built on RN's built-in `PanResponder` + `Animated`, so still **no `react-native-gesture-handler` / `reanimated` dependency**. Descendant gesture views (SwipeableRow tiles, horizontal lists) win the responder negotiation first, so neither gesture steals from them; the content swipe is also suppressed while the drawer is open and is a no-op on non-primary (detail) screens.
