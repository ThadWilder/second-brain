...
export function TaskCheckbox({ taskId, checked, onComplete }: TaskCheckboxProps) {
  ...
  useEffect(() => {
    if (!showPlated) return
    const timer = setTimeout(() => setShowPlated(false), 1500)
  ...
  return (
    <span className="relative inline-flex items-center">
      ...
      {showPlated && (
        <span className="absolute left-full ml-1.5 whitespace-nowrap text-[11px] text-green-700 font-medium animate-fade-in">
          Plated ✓
        </span>
      )}
    </span>
