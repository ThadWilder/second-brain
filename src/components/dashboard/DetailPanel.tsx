...
export function DetailPanel({ open, onClose, title, children }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      {/* Panel */}
      <div
        className="absolute top-0 right-0 h-full w-full max-w-[480px] bg-[var(--surface)] border-l border-[var(--border)] shadow-2xl flex flex-col animate-panel-slide-in"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--border)] shrink-0">
          <h2 className="text-base font-semibold text-[var(--text)] truncate pr-3">
            {title}
          </h2>
          <button
            onClick={onClose}
